...
const ConfigErrorScreen: React.FC = () => (
  <div className="min-h-screen bg-brand-900 flex items-center justify-center p-6 text-center">
    <div className="max-w-md w-full bg-white rounded-[3rem] p-10 shadow-2xl animate-fadeIn border-t-8 border-brand-500">
      ...
      <h2 className="text-2xl font-black text-gray-900 uppercase tracking-tighter mb-4 leading-tight">Quase lá!</h2>
      <p className="text-gray-500 text-sm font-medium leading-relaxed mb-8">
        O aplicativo detectou que as chaves do Supabase ainda não foram coladas no arquivo <code className="bg-gray-100 px-1 rounded">lib/supabase.ts</code>.
      </p>
      <div className="bg-gray-50 p-6 rounded-2xl text-left border border-gray-100 space-y-3 mb-8">
        <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Siga estes passos:</p>
        <div className="flex items-start gap-3">
            ...
            <p className="text-xs text-gray-600 font-bold">Vá em <span className="text-brand-600">Project Settings &gt; API</span> no seu Supabase.</p>
        </div>
        <div className="flex items-start gap-3">
            ...
            <p className="text-xs text-gray-600 font-bold">Copie a <span className="text-brand-600">Project URL</span> e a <span className="text-brand-600">anon public key</span>.</p>
        </div>
        <div className="flex items-start gap-3">
            ...
            <p className="text-xs text-gray-600 font-bold">Cole-as no arquivo <span className="text-gray-900 font-mono">lib/supabase.ts</span> entre as aspas.</p>
        </div>
      </div>
      <p className="text-[10px] text-gray-400 font-black uppercase italic">O app carregará automaticamente após você salvar.</p>
    </div>
  </div>
);
...
function App() {
  // Se não estiver configurado, mostra a tela de erro informativa
